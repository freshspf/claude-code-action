...
// 1. 评论获取错误
try {
  const comment = await fetchComment(commentId, isPRReviewComment);
} catch (error) {
  // 尝试另一种 API
  // 记录调试信息
  // 获取 PR 信息辅助调试
...
try {
  const outputData = JSON.parse(await fs.readFile(outputFile, "utf8"));
  executionDetails = extractExecutionDetails(outputData);
} catch (error) {
...
// 4. 评论更新错误
try {
  await updateClaudeComment(octokit.rest, updateParams);
} catch (updateError) {
  console.error(`Failed to update comment:`, updateError);
  throw updateError; // 重新抛出，因为这是关键功能
}
```
...
// 只在有实际变更时生成 PR 链接
if (comparison.total_commits > 0 || comparison.files?.length > 0) {
  prLink = generatePRLink(claudeBranch, baseBranch, context);
}
...
export function detectContentType(content: any): string {
  ...
  // 代码类型检测
  const codeKeywords = ["def ", "function ", "const ", "let "];
  if (codeKeywords.some(keyword => contentStr.includes(keyword))) {
    return detectProgrammingLanguage(contentStr);
  }
  
  // Shell 输出检测
  const shellIndicators = ["$ ", "Error:", "/"];
  if (shellIndicators.some(indicator => contentStr.includes(indicator))) {
    return "bash";
  }
...
2. **PR 链接无效**
   ...
   # 错误：URL 编码问题
   # 原因：特殊字符未正确编码
   # 解决：使用 ensureProperlyEncodedUrl 函数
   ```
...
```bash
# 查看评论 ID 和类型
echo "Comment ID: $CLAUDE_COMMENT_ID"
...
# 验证分支状态
git ls-remote origin $CLAUDE_BRANCH
