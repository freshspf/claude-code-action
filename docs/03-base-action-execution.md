...
1. **环境变量缺失**
   ```bash
   # 错误：ANTHROPIC_API_KEY is required
   # 解决：设置正确的 API Key
   ```

2. **Prompt 文件不存在**
   ```bash
   # 错误：Prompt file '/path/to/file' does not exist
   # 解决：检查文件路径和权限
   ```

3. **Claude CLI 安装失败**
   ```bash
   # 错误：Error spawning Claude process
   # 解决：检查 bun install -g @anthropic-ai/claude-code
   ```

4. **进程超时**
   ```bash
   # 错误：Claude process timed out after 600 seconds
   # 解决：增加 timeout_minutes 或优化 Prompt
   ```
...
# 查看执行日志
cat $RUNNER_TEMP/claude-execution-output.json

# 监控进程
ps aux | grep claude

# 检查命名管道
ls -la $RUNNER_TEMP/claude_prompt_pipe
```
